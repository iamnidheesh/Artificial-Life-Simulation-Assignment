...
def initialise() :
	...
	for i in range(200,620,20) :
		for j in range(200,620,20) :
	...
	for i in range(0,22,2) :
		...
		ind.append(i)
		if(i < 10) :
			mclamps.append(points[i])
			points[i].x = points[i].oldx + (10-i)*0.1
...
def updatePoints() :
	...
	for p in points :
		...
		if(p in clamps and p  not in mclamps) :
			continue
		...
		if(abs(mclamps[0].x-clamps[len(clamps)-1].x) < 0.2) :
			for i in mclamps :
				i.x = i.oldx
			index = len(mclamps)
			mclamps = []
			for m in range(10,22,2) :
				if(m < 16) :
					mclamps.append(points[m])
					points[m].x = points[m].oldx + (16-m)*0.1
